...
% dollars-dollars
%%
% where dollars\betadollars and dollars\mudollars are respectively the fertility and mortality rate.
% The parameter dollars\lambda>0dollars is an immigration term (or unstable term),
% dollarsQ_{\omega}=(a,b)\times (a_1,a_2)\times (0,T)dollars is the control domain with
% dollars(a,b)\subset (0,1)dollars and dollars(a_1,a_2)\subset (0,A).dollars
% Here dollars(a,b)=(0,1)dollars and dollars(a_1,a_2)=(0,A)dollars and we take dollarsA=1dollars.
%%
% We need to reduce the PDE to the finite dimensional system of the form
% dollars-dollars\dot{Z}=AZ+BUdollars-dollars
% where dollarsAdollars and dollarsBdollars are matrices, and 
%% 
% dollars-dollars
% \begin{pmatrix}
% Z(t) \\
...
% We suppose that dollars\lambda=\frac{1}{2}.dollars
clc; clear;
m=4;
n=9*m; 
%%
% Matrix of birth
% Here we suppose that fertility rate dollars\betadollars equal to:
%%
% dollars-dollars
% \beta(a) = 
% \begin{cases}
...
% dollars-dollars
%%
% Here dollars\alpha=7dollars, dollars\gamma=5dollars, dollarsv=3dollars, dollarsa_1=\frac{A}{9}dollars
%%
% The density of new born individuals will be approximated by:
%%
% dollars-dollarsy(x,0,t)=\int_{0}^{1}\beta(a)y(x,a,t)da=(1/n)\sum_{1}^{n}\beta(a_i)y(x,a_i,t)dollars-dollars
%%
% where dollarsa_idollars is the age discretization.
...
%%
% Mortality matrix.
% The mortality rate dollars\beta(a)=\dfrac{1}{9(A-a)}dollars
...
%% Initial condition
% We suppose that dollarsy(x,a,0)=e^{-((a-3/10)^2/\sqrt{2}+100(x-4/10))}dollars
T=10;
Z0=zeros(n*n,1);
for i=1:n
    for j=1:n
...
% linear system generated by the matrices dollars(A,B)dollars and an infinite time
...
% dollars-dollars
%%
% the feedback control law that minimizes the value of the cost is dollarsu =
% -Kzdollars, where dollarsK = R^{-1}B*Pdollars and dollarsPdollars is found by solving the continuous
% time algebraic Ricatti equation
%%
% dollars-dollars A*P+PA-(PB)R^{{-1}}(B*P)+Q=0.dollars-dollars
