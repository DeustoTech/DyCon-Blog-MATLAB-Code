...
% dollars\theta_i(t)dollars, dollarsi=1,\ldots,Ndollars and we want to compute a control such that
% in time dollarsTdollars we reach the state dollars\theta_i(T)=\ldots =\theta_N(T)=:\bar{\theta}dollars 
% in which all the oscillators are sincronized.
%% The mathematical model
% The dynamics of the oscillators is described by the Kuramoto model
%%
% dollars-dollars\dot{\theta}_i(t) = \frac{1}{N}\sum_{j=1}^N
% \sin(\theta_j(t)-\theta_i(t)) + b_i(t),\;\;\; i=1,\ldots,Ndollars-dollars
%%
%
% dollars-dollars \theta_i(0) = \theta_i^0.dollars-dollars
%%
% The model is non-linear, then, we apply a standard linearization process
% around the steady state dollars\bar{\theta}dollars obtaining
%%
% dollars-dollars\dot{\Theta}(t) =
% A\Theta(t),\;\;\;\Theta(0) = \Theta^0,\;\;\;\Theta =(\theta_1,\ldots,\theta_N)^T.dollars-dollars
%%
...
%%
% dollars-dollars J(b_i) = \frac{1}{2N}
% \sum_{j=1}^N(\theta_j(T)-\theta_i(T))^2+\frac{\beta}{2}\int_0^T
% b_i(t)^2dt,\;\;\;\beta>0,dollars-dollars
%%
% in which 
%%
...
% dollarsb = (b_1,\ldots,b_N)dollars which has minimum norm among all the controls 
...
% J(b_i)dollars too. Therefore, in each iteration of the Gradient Descent algorithm we shall
